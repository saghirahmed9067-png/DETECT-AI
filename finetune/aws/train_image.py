...
AWS g5.xlarge (A10G 24GB) — runs after audio OR simultaneously on separate instance
...
CHECKPOINT_DIR    = os.path.expanduser('~/aiscern-finetune/image-checkpoints')
IMG_SIZE          = 224
BATCH_SIZE        = 64        # A10G 24GB → can double from Colab's 32
EPOCHS            = 10
LR                = 2e-4
SEED              = 42
SAMPLES_PER_CLASS = 39000     # all available (78k ÷ 2)
...
def normalise_label(val):
    ...
    if s in ('FAKE','AI','1','GENERATED','DEEPFAKE','ARTIFICIAL'): return 1
    if s in ('REAL','HUMAN','0','AUTHENTIC','PHOTO'):              return 0
    return -1
...
for ds_id in [
    ...
    try:
        ...
        img_col = next((c for c in ds.column_names if c.lower() in ('image','img','photo')), None)
        lbl_col = next((c for c in ds.column_names if 'label' in c.lower()), None)
        if not img_col or not lbl_col: continue
...
# r=32 on A10G (doubled from Colab's 16 — better accuracy)
...
log.info(f"Training: {len(train_ds):,} × {EPOCHS} epochs | ~1.3h on A10G")
trainer.train(resume_from_checkpoint=last_ckpt)

results = trainer.evaluate()
log.info(f"Accuracy: {results['eval_accuracy']:.4f}  F1: {results['eval_f1']:.4f}")
trainer.push_to_hub(commit_message=f"Image detector — acc={results['eval_accuracy']:.4f}")
processor.push_to_hub(PUSH_REPO, token=HF_TOKEN)
log.info(f"✅ https://huggingface.co/{PUSH_REPO} | Cost: ~${1.3 * 0.302:.2f}")
